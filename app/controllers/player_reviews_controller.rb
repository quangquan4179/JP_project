class PlayerReviewsController < ApplicationController
  ...
  def create
    ...
    @player_review.user_id = current_user.id
  ...
  def update
    respond_to do |format|
      if @player_review.update(player_review_params)
        format.html { redirect_to @player_review, notice: "Player review was successfully updated." }
  ...
  private
    ...
    def player_review_params
      params.require(:player_review).permit(:user_id, :player_id, :review)
    end
end
